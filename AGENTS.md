...
3. **Draft/Modify Wiki Pages**:
   ...
   - When ingesting **external** content (URL, video, article — anything not already in the vault), also save a human-readable copy to `notes/ingested/` as `YYYY-MM-DD <Title>.md` tagged `#ingested`, so the user has a pleasant reading copy. Cite this copy alongside the original in the page's `sources:` field.
