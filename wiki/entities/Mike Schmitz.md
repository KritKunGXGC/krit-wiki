---
title: "Mike Schmitz"
category: "Entity"
tags: ["author", "obsidian", "pkm"]
sources: ["raw/8 Obsidian Plugins That I Can't Live Without.md"]
last_updated: 2026-08-12
---

# Mike Schmitz

Mike Schmitz เป็นครีเอเตอร์สาย PKM (Personal Knowledge Management) — เจ้าของช่อง YouTube **Practical PKM** และเว็บ [practicalpkm.com](https://practicalpkm.com/) ที่เผยแพร่คู่มือการสร้างระบบโน้ต/งาน/เวลาใน Obsidian แบบอิง workflow

## ภาพรวม
- **พื้นหลัง:** ใช้ Obsidian "all day, every day" — ลองใช้ plugins มากกว่าร้อยตัว, ติดตั้งจริง 39 ตัว
- **ธีมเนื้อหา:** plugin hygiene, PKM stack framework, multiscale planning (Cal Newport), digital journaling, plain-text task management
- **ผลิตภัณฑ์:** Starter Vault ฟรี (vault.practicalpkm.com) + LifeHQ (done-for-you vault เสียเงิน)

## Contributions in This Wiki
- Author of [[wiki/sources/8 Obsidian Plugins That I Can't Live Without|8 Obsidian Plugins That I Can't Live Without]] — 8 plugins S-tier จาก 39 ตัว: Calendar, Periodic Notes, Dataview, Keep the Rhythm, Templater, QuickAdd, Tasks, Actions URI

## ความคิดหลักที่แชร์กับ vault นี้
- **"ใช้ plugins ให้น้อยที่สุด" ≠ "ติดตั้งน้อยที่สุด"** — เขาติดตั้ง 39 ตัว แต่ essential แค่ 8 — แยก "มี" กับ "พึ่งพาจริง" ออกจากกัน (ดู [[wiki/syntheses/Essential Obsidian Plugins|Essential Obsidian Plugins]])
- **PKM stack:** เลือกเครื่องมือ/plugins ตามว่า fit กับ workflow อย่างไร ไม่ใช่ตามกระแส — หลักการเดียวกับที่ vault นี้คัด plugins ตามความจำเป็นจริง
- **Bases > Dataview:** สนับสนุนการย้ายไป [[wiki/concepts/Bases|Bases]] (Obsidian 1.9) สำหรับงานส่วนใหญ่ — สอดคล้องกับ vault นี้ที่ใช้ Bases อยู่แล้ว

## Connections
- [[wiki/concepts/Bases|Bases]] · [[wiki/concepts/Obsidian|Obsidian]] · [[wiki/concepts/MOC (Map of Content)|MOC]]
- [[wiki/entities/Tim Miller|Tim Miller]] — ผู้เขียนเนื้อหา PKM/Obsidian เช่นกัน
- [[wiki/MOCs/Obsidian & PKM|Obsidian & PKM]] — MOC กลุ่มเนื้อหานี้

## Sources
- `raw/8 Obsidian Plugins That I Can't Live Without.md`
